...
<template>
  <div class="common-layout">
    <ImageCropper v-if="cropper" :src="user.avatar" @submit="editAvatar" />
          ...
          <el-container class="page-main fade-down" direction="vertical">
            <section class="page-title">
              <div class="info-title">
                <img class="user-pic" :src="user.avatar" @click="cropper = true;" />
                <div class="user-info">
                  <div id="info">
                    <span id="h-name">{{ user.name }}</span>
                    <span id="h-gender" class="icon-general" :style="{ '--gender': 0 }"></span>
                    <span id="h-level" class="icon-level" :style="{ '--lv': user.level }"></span>
                  </div>
                  <input id="signature" v-model="user.signature" autocomplete="off" placeholder="编辑个性签名"
                    @blur="editSignature" />
                </div>
              </div>
            </section>
