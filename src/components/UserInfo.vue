...
<template>
  <div id="userinfo" class="fade-in" v-if="user">
    <img class="userinfo-image" :src="user.profile" v-if="user.profile != ''" />
    <img class="userinfo-image" :src="defaultProfile" v-else />
    <div class="userinfo-block fade-in" v-loading="loading">
      <div class="close-btn" @click="close">&times;</div>
      <div class="userinfo-info">
        <div id="name">{{ user.username }}</div>
        <div id="signature">{{ user.signature }}</div>
        <hr class="title-hr" />
        <table class="stats-table">
          <tr>
            <td class="label">总积分</td>
            <td class="value">{{ user.credit }}</td>
          </tr>
        </table>
        <hr class="stats-hr" />
        <table class="stats-table">
  ...
  watch: {
    username: {
      handler(newVal) {
        if (newVal != "") {
          this.loading = true;
          AuthService.userInfo(newVal).then(
            (content) => {
              this.user = content;
